...
import com.liux.android.multimedia.inject.InjectFragment;
...
public class MultimediaUtil {
    ...
    public static File getCacheDir(Context context) {
        ...
        File cache = new File(cacheDir, "multimedia");
        cache.mkdirs();
        cache.mkdir();
        return cache;
    }
    ...
    public static InjectFragment getInjectFragment(FragmentActivity fragmentActivity) {
        InjectFragment fragment = (InjectFragment) fragmentActivity.getSupportFragmentManager().findFragmentByTag(Multimedia.TAG);
    ...
    public static Uri getProviderUri(Context context, File file) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            return MultimediaFileProvider.getUriForFile(
                    context,
                    context.getPackageName() + ".MultimediaFileProvider",
                    file
            );
        }
